...
// 10k: optional setting to unlock wallet for block minting only;
...
// 10k: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // 10k: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // 10k: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    if (!GetBoolArg("-avatar")) // 10k: not avatar mode
                    {
...
// 10k: create coin stake transaction
...
// 10k: check 'spent' consistency between wallet and txindex
// 10k: fix wallet spent state according to txindex
...
// 10k: disable transaction (only for coinstake)
