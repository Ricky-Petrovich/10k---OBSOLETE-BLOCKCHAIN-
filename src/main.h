...
static const int STAKE_START_TIME = 1422403200;
static const int STAKE_TARGET_SPACING = 30;
static const int STAKE_MIN_AGE = 60 * 60;
static const int STAKE_MAX_AGE = 60 * 60 * 24 * 16;
...
public:
    ...
    {
        // 10k: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64& nCoinAge) const;  // 10k: get transaction coin age
...
public:
    ...
    // 10k: block signature - signed by coin base txout[0]'s owner
    ...
    // 10k: two types of block: proof-of-work or proof-of-stake
    ...
    // 10k: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64& nCoinAge) const; // 10k: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // 10k: entropy bit for stake modifier if chosen by modifier
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    CBigNum bnChainTrust; // 10k: trust score of block chain
    int nHeight;
    int64 nMint;
    int64 nMoneySupply;

    unsigned int nFlags;  // 10k: block index flags
    enum  
    {
